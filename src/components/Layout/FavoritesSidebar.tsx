...
const FavoritesSidebar: React.FC<FavoritesSidebarProps> = ({ onShowLogin }) => {
  ...
  return (
    <div
      className={`fixed right-0 top-20 z-40 h-[calc(100vh-120px)] transition-all duration-300 ease-out ${
        isExpanded ? "w-[320px]" : "w-14"
      }`}
    >
      {/* 통합 사이드바 - 매트 글라스 */}
      <div className="h-full rounded-l-2xl glass-matte flex">
        {/* 왼쪽 아이콘 탭 바 */}
        <div className="relative z-10 w-14 shrink-0 border-r border-slate-200/40 flex flex-col items-center py-3 gap-1">
          {/* 접기/펼치기 버튼 */}
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            aria-label={isExpanded ? "사이드바 접기" : "사이드바 펼치기"}
            className="w-10 h-8 flex items-center justify-center text-slate-400 hover:text-slate-600 mb-2 transition-colors"
          >
            {isExpanded ? (
          ...
          <div className="w-8 border-t border-blue-100/80 mb-2" />
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`w-10 py-2.5 flex flex-col items-center gap-1 rounded-xl transition-all ${
                  isActive && isExpanded
            ...
            {!isAuthenticated ? (
                ...
                <ul>
                  {favoritesList.map((item) => (
                    <li key={item.companyId}>
                      <div className="w-full px-4 py-3 flex items-center gap-3 hover:bg-blue-100/80 transition-all border-b border-slate-200/30">
                        {/* 클릭 가능한 기업 정보 영역 */}
                        <button
                        ...
                        >
                          {/* 로고 */}
                          <div className="w-9 h-9 rounded-lg bg-white/70 border border-white/60 overflow-hidden flex items-center justify-center shrink-0">
                            {item.logoUrl ? (
                              <img
                                src={item.logoUrl}
                        ...
                        <button
                          ...
                          className="p-1.5 hover:bg-red-100/50 rounded-full transition-all shrink-0"
                          title="관심 해제"
              ...
              isComparisonsLoading ? (
                ...
                <div className="p-3 space-y-2">
                  {comparisonDetails.map((query, index) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full p-3 bg-white/30 hover:bg-blue-100/80 rounded-xl transition-all text-left group"
                      >
                        {/* 세트 이름 */}
                          ...
                          {isDetailLoading ? (
                            ...
                            comparison.companies.slice(0, 4).map((company) => (
                              <span
                                ...
                                className="px-2 py-0.5 bg-white/60 border border-white/80 rounded-full text-xs text-slate-600 truncate max-w-20"
                              >
                                {company.companyName}
                              </span>
                            ))
                  ...
                  <button
                    onClick={() => navigate("/compare")}
                    className="w-full p-3 border border-dashed border-slate-300/60 hover:border-blue-400 hover:bg-blue-50/40 rounded-xl transition-all flex items-center justify-center gap-1 text-slate-400 hover:text-blue-500"
                  >
                    <ArrowLeft size={16} />
              ...
              isVisitsLoading ? (
                ...
                <div className="flex flex-col h-full">
                  {/* 전체 삭제 버튼 */}
                  <div className="px-4 py-2 border-b border-slate-200/50 flex justify-end">
                    <button
                      ...
                      <li key={item.visitId}>
                        <div className="w-full px-4 py-3 flex items-center gap-3 hover:bg-blue-100/80 transition-all border-b border-slate-200/30">
                          ...
                          <button
                            ...
                            className="p-1.5 hover:bg-slate-200/50 rounded-full transition-all shrink-0"
                            title="삭제"
