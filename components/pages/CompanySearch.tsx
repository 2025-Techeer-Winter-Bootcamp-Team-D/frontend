...
import { Search, Star, TrendingUp, Filter, ChevronRight, ArrowRight, User } from 'lucide-react';
...
const RANKING_DATA = [
    { rank: 1, name: '삼성전자', code: '005930', sector: '반도체', price: '73,400', change: '+0.96%', changeVal: 700, marketCap: '430조' },
    { rank: 2, name: 'SK하이닉스', code: '000660', sector: '반도체', price: '164,500', change: '+2.10%', changeVal: 3400, marketCap: '119조' },
    { rank: 3, name: 'LG에너지솔루션', code: '373220', sector: '2차전지', price: '385,000', change: '-1.50%', changeVal: -5000, marketCap: '90조' },
    { rank: 4, name: '삼성바이오로직스', code: '207940', sector: '바이오', price: '812,000', change: '+2.50%', changeVal: 19000, marketCap: '57조' },
    { rank: 5, name: '현대차', code: '005380', sector: '자동차', price: '245,000', change: '-1.20%', changeVal: -3000, marketCap: '51조' },
    { rank: 6, name: '기아', code: '000270', sector: '자동차', price: '112,000', change: '-0.80%', changeVal: -900, marketCap: '44조' },
    { rank: 7, name: '셀트리온', code: '068270', sector: '바이오', price: '185,000', change: '+1.80%', changeVal: 3300, marketCap: '40조' },
    { rank: 8, name: 'POSCO홀딩스', code: '005490', sector: '철강', price: '420,000', change: '+0.50%', changeVal: 2000, marketCap: '35조' },
    { rank: 9, name: '신한지주', code: '055550', sector: '금융', price: '78,200', change: '+0.51%', changeVal: 400, marketCap: '40조' },
    { rank: 10, name: 'KB금융', code: '105560', sector: '금융', price: '72,100', change: '+1.12%', changeVal: 800, marketCap: '38조' },
    { rank: 11, name: 'NAVER', code: '035420', sector: '서비스', price: '185,500', change: '-0.30%', changeVal: -500, marketCap: '30조' },
    { rank: 12, name: '삼성SDI', code: '006400', sector: '2차전지', price: '385,000', change: '+0.20%', changeVal: 1000, marketCap: '26조' },
    { rank: 13, name: 'LG화학', code: '051910', sector: '화학', price: '450,000', change: '-1.10%', changeVal: -5000, marketCap: '31조' },
    { rank: 14, name: '카카오', code: '035720', sector: '서비스', price: '52,100', change: '+0.80%', changeVal: 400, marketCap: '23조' },
    { rank: 15, name: '하나금융지주', code: '086790', sector: '금융', price: '58,400', change: '-0.32%', changeVal: -200, marketCap: '28조' },
];

const CompanySearch: React.FC<CompanySearchProps> = ({ setPage, starred, onToggleStar, setCompanyCode }) => {
    ...
    const filteredList = useMemo(() => {
        ...
        return RANKING_DATA.filter(item => 
                    ...
                    <GlassCard className="p-0 overflow-hidden min-h-[600px] flex flex-col">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                                ...
                                {searchQuery ? `검색 결과 (${filteredList.length})` : '시가총액 상위 랭킹'}
                            </h2>
                                    ...
                                    {filteredList.map((item, index) => (
                                        ...
                                        >
                                            <td className="px-6 py-4 text-center">
                                                <span className={`font-bold ${index < 3 ? 'text-shinhan-blue text-lg' : 'text-slate-500'}`}>
                                                    {item.rank}
                                                </span>
                                            </td>
                                            ...
                                            <td className={`px-6 py-4 text-right font-bold ${item.change.startsWith('+') ? 'text-red-500' : 'text-blue-500'}`}>
                                                {item.change}
                                                <div className="text-[10px] font-normal opacity-70">
                                                    {item.change.startsWith('+') ? '▲' : '▼'} {Math.abs(item.changeVal).toLocaleString()}
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <ChevronRight size={18} className="text-gray-300 group-hover:text-shinhan-blue transition-colors" />
                                            </td>
                                        </tr>
                                    ))}
                        ...
                        <div className="flex-1 overflow-y-auto max-h-[600px] custom-scrollbar bg-white">
                            {starredList.length > 0 ? (
                                    ...
                                    {starredList.map(item => (
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                 <button 
                                                    onClick={(e) => { e.stopPropagation(); onToggleStar(item.code); }}
                                                    className="text-yellow-400 hover:text-yellow-500 transition-colors"
                                                >
                                                    <Star size={18} fill="currentColor" />
                                                </button>
                                                <div>
                                                    <div className="font-bold text-slate-800 text-sm group-hover:text-shinhan-blue transition-colors">{item.name}</div>
                                                    <div className="text-xs text-gray-400 font-mono">{item.code}</div>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <div className="font-bold text-slate-700 text-sm">{item.price}</div>
                                                <div className={`text-xs font-bold ${item.change.startsWith('+') ? 'text-red-500' : 'text-blue-500'}`}>
                                                    {item.change}
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div className="text-center py-10 px-4">
                                    ...
                                    <p className="text-slate-500 font-bold mb-1">관심 기업이 없습니다</p>
                                    <p className="text-xs text-gray-400 leading-relaxed">
                                        왼쪽 리스트에서 <Star size={10} className="inline mx-0.5 text-gray-400"/> 버튼을 눌러<br/>
                                        관심 기업을 추가해보세요.
                                    </p>
                                </div>
                            )}
