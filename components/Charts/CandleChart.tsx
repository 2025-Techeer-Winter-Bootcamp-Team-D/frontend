                        ...
                        if (active && payload && payload.length) {
                            ...
                            return (
                                <div className="bg-white/90 backdrop-blur p-3 rounded-lg shadow-xl border border-gray-100 text-xs">
                                    <div className="font-bold mb-1 text-slate-700">{data.time}</div>
                                    <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                                        <span className="text-gray-500">시가</span>
                                        <span className={`font-mono text-right ${data.open > data.close ? 'text-blue-500' : 'text-red-500'}`}>{data.open.toLocaleString()}</span>
                                        <span className="text-gray-500">고가</span>
                                        <span className="font-mono text-right text-red-500">{data.high.toLocaleString()}</span>
                                        <span className="text-gray-500">저가</span>
                                        <span className="font-mono text-right text-blue-500">{data.low.toLocaleString()}</span>
                                        <span className="text-gray-500">종가</span>
                                        <span className={`font-mono text-right ${data.close > data.open ? 'text-red-500' : 'text-blue-500'}`}>{data.close.toLocaleString()}</span>
                                    </div>
                                </div>
                            );
                        }
